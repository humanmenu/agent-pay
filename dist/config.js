...
export function configFromEnv() {
    ...
    const overrides = {};
    if (env.AGENT_PAY_RPC_URL)
        overrides.rpcUrl = env.AGENT_PAY_RPC_URL;
    if (env.AGENT_PAY_PRIVATE_KEY)
        overrides.privateKey = env.AGENT_PAY_PRIVATE_KEY;
    const chainId = parseChainId(env.AGENT_PAY_CHAIN_ID);
    if (chainId)
        overrides.chainId = chainId;
    if (env.AGENT_PAY_USDC_ADDRESS)
        overrides.usdcAddress = env.AGENT_PAY_USDC_ADDRESS;
    if (env.AGENT_PAY_MAX_PER_TX_USDC)
        overrides.maxPerTxUsdc = env.AGENT_PAY_MAX_PER_TX_USDC;
    if (env.AGENT_PAY_MAX_PER_DAY_USDC)
        overrides.maxPerDayUsdc = env.AGENT_PAY_MAX_PER_DAY_USDC;
    const allowlist = parseAllowlist(env.AGENT_PAY_ALLOWLIST_DOMAINS);
    if (allowlist)
        overrides.allowlistDomains = allowlist;
...
export async function resolveConfig() {
    ...
    const fileAssets = normalizeAssets(fileConfig?.assets);
    const envAssets = normalizeAssets(envConfig.assets);
    const mergedAssets = {
        ...defaultAssets,
        ...(fileAssets || {}),
        ...(envAssets || {}),
