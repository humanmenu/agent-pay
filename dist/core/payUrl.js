...
function parseHeaderInvoice(res) {
    const primary = res.headers.get("payment-required");
    if (primary)
        return parseInvoiceFromHeader(primary);
    const legacy = res.headers.get("x-payment-required");
    if (legacy)
        return parseInvoiceFromHeader(legacy);
    return null;
}
...
function resolveTimeoutSeconds(invoiceTimeout, userTimeout) {
    if (userTimeout === undefined)
        return invoiceTimeout;
    return Math.min(userTimeout, invoiceTimeout);
}
export async function payUrl(url, options, config) {
    ...
    const initialResponse = await fetchWithTimeout(targetUrl.toString(), {
        method,
        headers,
        body,
    ...
    const validated = validateInvoice(invoice, config.assets || { [config.usdcAddress || DEFAULT_USDC_ADDRESS]: { decimals: 6 } });
    ...
    const proofHeader = typeof invoice.proof?.header === "string" && invoice.proof.header.trim()
