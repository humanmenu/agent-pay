...
function handleError(command: string, err: unknown, json: boolean) {
...
function requireYesWhenNonInteractive(isYes: boolean) {
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      if (json) {
        ...
        if (result.ethBalance) console.log(`ETH: ${result.ethBalance}`);
        if (result.usdcBalance) console.log(`USDC: ${result.usdcBalance}`);
        if (!result.ok) console.log("Doctor: issues detected");
...
program
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      let preBalance: string | undefined;
      ...
      const result = await payUrl(
        url,
        {
          ...
          onInvoice: async (details) => {
            ...
            if (!process.stdin.isTTY) return Boolean(opts.yes);
            if (opts.yes) return true;
            return promptYesNo(
              `Pay ${details.amountRequired} USDC to ${details.payTo} for invoice ${details.invoiceId}?`
            );
          },
        },
        config
