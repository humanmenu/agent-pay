...
export function parseInvoiceFromHeader(value: string): Invoice {
  const trimmed = value.trim();
  if (!trimmed) {
    throw new CliError("empty_invoice_header", "Invoice header is empty.");
  }

  const tryParse = (input: string): Invoice | null => {
    try {
...
export function parseInvoiceFromBody(bodyText: string): Invoice {
  try {
    return JSON.parse(bodyText) as Invoice;
  } catch {
    throw new CliError("invalid_invoice_body", "Invoice body is not valid JSON.");
  }
}

export function validateInvoice(invoice: Invoice, assets: Record<string, AssetConfig>): {
